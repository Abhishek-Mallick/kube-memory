...
| Surface | URL |
|---------|-----|
| Dashboard | [kube-memory.buildlab.in](https://kube-memory.buildlab.in/) |
| API / MCP | [kube.buildlab.in](https://kube.buildlab.in/) |
| MCP endpoint | `https://kube.buildlab.in/mcp` |
...
kube-memory **does not run inside your cluster**. The prod server runs on **Vercel** (`kube.buildlab.in`). When you click **Test connection** in the dashboard, **Vercel** makes the outbound HTTP call — not your Mac.
...
1. Open [kube-memory.buildlab.in](https://kube-memory.buildlab.in/) → sign in
...
1. [kube-memory.buildlab.in](https://kube-memory.buildlab.in/) → **Integrations** → **Prometheus** → **Connect**
...
- [ ] `km_*` key in Cursor → `https://kube.buildlab.in/mcp`
...
1. **Dashboard (browser)** → JWT session → `PUT /connectors/kubernetes` or `PUT /connectors/prometheus` on `kube.buildlab.in`
...
4. **MCP tool call** (Cursor) → `POST https://kube.buildlab.in/mcp` with `Authorization: Bearer km_*`
...
|------|-----------|-------------------|
| Dashboard | `localhost:5173` | [kube-memory.buildlab.in](https://kube-memory.buildlab.in/) |
| MCP URL | `http://localhost:3000/mcp` | `https://kube.buildlab.in/mcp` |
...
- [usage.md](../usage.md) — MCP tool reference
