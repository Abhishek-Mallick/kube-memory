...
End-to-end **detect → diagnose → treat → remember → notify** using one orchestrated incident flow plus individual MCP tools.
...
| Cognee | Optional | `COGNEE_API_KEY` in `server/.env` for recall |
