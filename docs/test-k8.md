...
**MCP (prod):** `https://kube.buildlab.in/mcp`
...
| **B — Prod** | GKE / EKS / DO / etc. (public API) | `kube.buildlab.in` | Demo / team / hackathon judges |
...
1. [kube-memory.buildlab.in](https://kube-memory.buildlab.in/) → Integrations → Kubernetes → paste kubeconfig → Test → Save → **Enable**
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| **`ECONNREFUSED 127.0.0.1:55xxx`** with prod MCP URL | `mcp.json` points at `https://kube.buildlab.in/mcp` but kubeconfig has `server: https://127.0.0.1:…` (kind). Prod runs on Vercel — its `127.0.0.1` is **not** your Mac. | For kind: set MCP URL to `http://localhost:3000/mcp` and run `cd server && npm run dev`. **Or** use a cloud cluster for prod. |
