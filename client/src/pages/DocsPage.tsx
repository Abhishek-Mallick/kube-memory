...
const connectorSetup = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Expose the Prometheus server URL to kube-memory — ingress, internal cluster URL, or port-forward for local server.",
      "If Prometheus has no auth, leave bearer token empty. If behind a proxy/OAuth, paste the bearer token.",
      "Dashboard → Integrations → Prometheus → Base URL (e.g. https://prometheus.example.com) → Test → Save → Enable.",
  ...
  },
  {
    ...
    tools: "argocd_list_applications, argocd_get_application, argocd_get_app_history, argocd_sync_application, argocd_rollback_application",
    priority: "Recommended — GitOps deploy cycle",
    webhook: false,
    steps: [
      "Install ArgoCD in your demo cluster and create a demo Application (synced or deliberately OutOfSync).",
      "Generate an API token: ArgoCD UI → User settings → Accounts, or `argocd account generate-token`.",
      "Expose ArgoCD server URL (ingress or port-forward).",
      "Dashboard → Integrations → ArgoCD → Base URL + API token → Test → Save → Enable.",
    ],
